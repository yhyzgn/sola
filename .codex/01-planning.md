...
6. **[重构] 实现 GPUI 原生编辑器 Element**：参考 Zed 架构，将 Focused 编辑区从 Div 拼接模型重构为自定义 Element 驱动模型，实现像素级精确渲染。
...
4. **[原生重构] 采用自定义 Element 替代 Div-soup。**
   - 理由：对齐 Zed 官方实践，解决软换行、光标抖动和性能瓶颈，为出版级排版打下基础。
