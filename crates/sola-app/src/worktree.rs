...
impl Worktree {
    pub fn local(path: impl Into<PathBuf>, cx: &mut App) -> Entity<Self> {
        ...
        cx.spawn(|cx: &mut gpui::AsyncApp| {
            let mut cx = cx.clone();
            async move {
                let root_entry = cx.background_executor().spawn(async move {
                    scan_recursive(abs_path_initial)
                }).await;
...
fn build_tree_recursive(path: &Path, map: &mut std::collections::HashMap<PathBuf, Vec<Entry>>) -> Entry {
